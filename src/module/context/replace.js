...
export default class {
	...
	replace(texts, ranges, afterCursorPosList) {
		...
		historyArr = this.context._deleteMultiContent({
		...
		historyArr = this.context._insertMultiContent({
			text: texts,
			cursorPosList: historyArr.map((item) => {
				return item.cursorPos
			})
		});
		historyArr.serial = serial;
		historyArr.afterCursorPosList = afterCursorPosList || historyArr.map((item) => {
			return item.cursorPos
		});
		this.editor.history.pushHistory(historyArr);
	...
	joinLine() {
		...
		this.editor.cursor.multiCursorPos.forEach((item) => {
			let range = this.editor.selecter.getRangeByCursorPos(item);
			if (range && range.end.line > range.start.line) {
				ranges.push({
					start: {
						line: range.start.line,
						column: 0
		...
		function _getRanges() {
			...
			this.editor.cursor.multiCursorPos.forEach(cursorPos => {
				...
				range = {
					start: {
						line: cursorPos.line,
						column: cursorPos.column - word.length
					},
					end: {
						line: cursorPos.line,
